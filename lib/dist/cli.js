...
var jsonHasKeys = async (context, ruleOptions) => {
  ...
  const { path: path2, keys } = sanitizedRuleOptions;
  const paths = Array.isArray(path2) ? path2 : [path2];
  for (const pathPattern of paths) {
    const isPattern = isGlobPattern(pathPattern);
    let filesToCheck;
...
var ruleConfigSchema = import_zod12.default.union([
  FileContainsSchema,
...
var rulesConfigSchema = import_zod12.default.array(ruleConfigSchema);

// src/utils/config.ts
var regexPatternSchema = import_zod13.z.string().refine(
  (pattern) => {
    try {
      new RegExp(pattern);
      return true;
...
var filtersSchema = import_zod13.z.object({
  archived: import_zod13.z.boolean().optional(),
  organizations: import_zod13.z.array(import_zod13.z.string()).optional(),
  visibility: import_zod13.z.enum(["public", "private", "all"]).optional(),
  include: import_zod13.z.array(regexPatternSchema).optional(),
  exclude: import_zod13.z.array(regexPatternSchema).optional()
});
