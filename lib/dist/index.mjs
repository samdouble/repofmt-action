...
// src/utils/rule-schema.ts
var regexPatternSchema = z2.string().refine(
  (pattern) => {
    try {
      new RegExp(pattern);
      return true;
...
var exceptionsSchema = z2.array(regexPatternSchema).optional();
function createRuleSchema(name, optionsSchema) {
  return z2.object({
    name: z2.literal(name),
...
// src/rules/file-contains.ts
var FileContainsOptionsSchema = z3.object({
...
var EntryTypeSchema = z4.enum(["file", "directory", "any"]).default("file");
var FileExistsOptionsSchema = z4.object({
...
var EntryTypeSchema2 = z5.enum(["file", "directory", "any"]).default("file");
var FileForbiddenOptionsSchema = z5.object({
...
var FileNotContainsOptionsSchema = z6.object({
  path: z6.union([z6.string(), z6.array(z6.string()).min(1)]),
  contains: z6.string(),
  caseSensitive: z6.boolean().default(false)
});
...
var GithubActionsTimeoutMinutesOptionsSchema = z7.object({
  maximum: z7.number().positive().optional()
});
...
// src/rules/json-has-keys.ts
var JsonHasKeysOptionsSchema = z8.object({
  path: z8.union([z8.string(), z8.array(z8.string()).min(1)]),
  keys: z8.array(z8.string()).min(1)
});
...
var LicenseExistsOptionsSchema = z9.object({
  caseSensitive: z9.boolean().default(false),
  path: z9.union([z9.string(), z9.array(z9.string()).min(1)]).default("LICENSE.md")
});
...
var PyprojectDependenciesAlphabeticalOrderOptionsSchema = z10.object({
  path: z10.string().default("pyproject.toml"),
  sections: z10.array(z10.string()).default(["project.dependencies", "project.optional-dependencies", "tool.poetry.dependencies"])
});
...
var ReadmeExistsOptionsSchema = z11.object({
  caseSensitive: z11.boolean().optional().default(false),
  path: z11.union([z11.string(), z11.array(z11.string()).min(1)]).optional().default("README.md")
});
...
var ReadmeHasBadgesOptionsSchema = z12.object({
  path: z12.string().default("README.md"),
  minCount: z12.number().int().min(0).optional(),
  patterns: z12.array(z12.string()).optional()
});
...
var ReadmeHasSectionOptionsSchema = z13.object({
  section: z13.string(),
  path: z13.string().optional().default("README.md"),
  caseSensitive: z13.boolean().optional().default(false)
});
...
// src/rules/requirements-txt-dependencies-alphabetical-order.ts
var RequirementsTxtDependenciesAlphabeticalOrderOptionsSchema = z14.object({
  path: z14.string().default("requirements.txt")
});
...
var YamlHasKeysOptionsSchema = z15.object({
  path: z15.union([z15.string(), z15.array(z15.string()).min(1)]),
  keys: z15.array(z15.string()).min(1)
});
...
// src/rules/index.ts
import z16 from "zod";
var ruleConfigSchema = z16.union([
  FileContainsSchema,
...
var rulesConfigSchema = z16.array(ruleConfigSchema);

// src/utils/config.ts
var regexPatternSchema2 = z17.string().refine(
  (pattern) => {
    try {
      new RegExp(pattern);
      return true;
...
var filtersSchema = z17.object({
  archived: z17.boolean().optional(),
  organizations: z17.array(z17.string()).optional(),
  visibility: z17.enum(["public", "private", "all"]).optional(),
  include: z17.array(regexPatternSchema2).optional(),
  exclude: z17.array(regexPatternSchema2).optional()
});
var configSchema = z17.object({
